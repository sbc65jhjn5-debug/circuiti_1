...
def angoli(I, N, L, r, B_t):
    return np.arctan(tan_angoli(I, N, L, r, B_t))


def sigma_theta_corrente(sigma_corr, I, B_fit, r, L, N):
    coeff = (4 * np.pi * 1e-7) * N / (2 * B_fit * np.sqrt(r**2 + (L / 2)**2))
    dtheta_dI = coeff / (1 + (coeff * I)**2)
    return dtheta_dI * sigma_corr
...
ls1 = LeastSquares(I, theta, sigma_theta, angoli)
...
sigma_tot = np.sqrt(sigma_theta_corrente(sigma_corrente, I, B_fit, raggio, lunghezza, N=31)**2 + sigma_theta**2)

ls2 = LeastSquares(I, theta, sigma_tot, angoli)
...
ax.errorbar(I, deg,
            yerr=np.degrees(sigma_tot),
            xerr=sigma_corrente, # DA INSERIRE IL VALORE CORRETTO,
            marker="o",
            linestyle="None",
            capsize=4,
            color="indigo",
...
I_fit = np.linspace(np.min(I), np.max(I), 300)
ax.plot(I_fit, np.degrees(angoli(I_fit, 31, lunghezza, raggio, B_fit_2)),
        color="magenta",
        label="Fit non lineare")
