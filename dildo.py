...
# Fit in configurazione 2

sigma_V_2 = 0.0005 # V
sigma_tot_conf2 = sigma_I_conf2.copy ()

for _ in range (8):
    ls2 = LeastSquares (V_2, 
                       I_2,
                       sigma_I_conf2,
                       I_shockley)
    ...
    derivata_I_2 = dI_dV_shockley (V_2, m2.values["I_0"], m2.values["g"])
    sigma_tot_conf2 = np.sqrt (sigma_I_conf2**2 + (derivata_I_2 * sigma_V_2)**2)
...
I_0_fit_2 = m2.values["I_0"]
g_fit_2 = m2.values["g"]

I_fit_punti_2 = I_shockley (V_2, I_0_fit_2, g_fit_2)
n_parametri_fit_2 = len (m2.parameters)

chi2_lineare = np.sum (((I_2 - I_fit_punti_2) / sigma_tot_conf2) ** 2)
ndof_lineare = len (I_2) - n_parametri_fit_2
p_value_lineare = chi2.sf (chi2_lineare, ndof_lineare)
...
mask_log = (I_2 > 0) & (I_fit_punti_2 > 0)
sigma_log_I_2 = sigma_tot_conf2[mask_log] / I_2[mask_log]
chi2_log = np.sum (((np.log (I_2[mask_log]) - np.log (I_fit_punti_2[mask_log])) / sigma_log_I_2) ** 2)
ndof_log = np.sum (mask_log) - n_parametri_fit_2
p_value_log = chi2.sf (chi2_log, ndof_log)
...
x_axis_2 = np.linspace (0.07, max(V_2) + 0.01, 5000)
y_fit_2 = I_shockley (x_axis_2, I_0_fit_2, g_fit_2)
y_fit_2_log = np.where (y_fit_2 > 0, y_fit_2, np.nan)
...
ax[0].errorbar (V_2, I_2,
             xerr = sigma_V_2,
             yerr = sigma_tot_conf2,
             color = "indigo",
             linestyle = "None",
             marker = 'o',
             capsize = 4,
